...
export function buildLayoutPage(prefs, window, settings, createResetBtn, createGroupReset) {
    ...
    addSwitchRow(prefs, posGroup, settings, 'show-on-all-monitors', 'Show on All Monitors', 'Display the dock on every connected screen', 'video-display-symbolic', null);
    addSwitchRow(prefs, posGroup, settings, 'isolate-monitors', 'Isolate Monitors', 'Only show apps running on the current monitor', 'video-display-symbolic', null);

    addSwitchRow(prefs, posGroup, settings, 'independent-dock', 'Independent Dock Mode', 'Use completely separate pinned apps and custom app launcher', 'system-run-symbolic', null);
    const showIndepOverviewRow = addSwitchRow(prefs, posGroup, settings, 'show-independent-in-overview', 'Show Independent Dock in Overview', 'Shows dock on the left side during overview', 'view-grid-symbolic', null);
    const fullWidthRow = addSwitchRow(prefs, posGroup, settings, 'full-width', 'Full Screen Width', 'Extend dock edge to edge', 'view-fullscreen-symbolic', null);
    ...
    addCustomSpinRow(prefs, posGroup, settings, 'dock-margin', 'Edge Margin', 'Distance from screen edge', 'format-indent-less-symbolic', {
        lower: 0,
        upper: 50,
        step_increment: 1
    }, createResetBtn);
    ...
    addCustomSpinRow(prefs, sizeGroup, settings, 'icon-size', 'Base Icon Size', 'Normal size of app icons', 'zoom-original-symbolic', {
        lower: 16,
        upper: 128,
        step_increment: 2
    }, createResetBtn);
    addCustomSpinRow(prefs, sizeGroup, settings, 'icon-spacing', 'Icon Gap', 'Distance between icons', 'format-indent-more-symbolic', {
        lower: 0,
        upper: 30,
        step_increment: 1
    }, createResetBtn);
    const sidePaddingRow = addCustomSpinRow(prefs, sizeGroup, settings, 'dock-padding', 'Side Padding', 'Extra gap inside dock ends', 'format-justify-fill-symbolic', {
        lower: 0,
        upper: 150,
        step_increment: 2
    }, createResetBtn);

    addCustomSpinRow(prefs, sizeGroup, settings, 'dock-height', 'Dock Height / Thickness', 'Extra padding on top/bottom (or left/right)', 'format-justify-center-symbolic', {
        lower: 0,
        upper: 100,
        step_increment: 2
    }, createResetBtn);
