...
import { DockThemes } from './Themes.js';
...
export default class AppContextMenu {
    constructor(dockUI, app, buttonActor, isCtrlPressed = false, openPrefsCallback = null) {
        ...
        this.actor.connect('button-release-event', () => { this.hide(); return Clutter.EVENT_STOP; });
        ...
        this.panel = new St.BoxLayout({ vertical: true, reactive: true, style_class: 'context-menu-panel' });
    ...
    _buildMenu() {
        ...
        if (windows.length > 0) {
            ...
            const thumbSpacing = 12, panelPadding = 24;
            ...
            const thumbScroll = new St.ScrollView({
                ...
                enable_mouse_scrolling: true, overlay_scrollbars: true, style_class: 'context-menu-thumb-scroll'
            });
            ...
            windows.forEach(win => {
                ...
                if (compPrivate) {
                    ...
                    let thumbW = customSize, thumbH = (h / w) * thumbW;
                ...
                const controlsBox = new St.BoxLayout({ vertical: false, opacity: 0, reactive: true, style_class: 'context-menu-controls-box' });

                if (!win.minimized) {
                    controlsBox.add_child(this._createWindowControl('window-minimize-symbolic', '255, 189, 46', () => {
                        this.hide(); animateMinimize(win, this.buttonActor, this.dockUI.dockPosition);
                    }));
                }
                ...
                controlsBox.add_child(this._createWindowControl(win.minimized ? 'view-fullscreen-symbolic' : (isMaximized ? 'window-restore-symbolic' : 'window-maximize-symbolic'), '40, 201, 64', () => {
                    this._previousFocus = null; this.hide();
                    win.activate(global.get_current_time());
                    
                    if (win.minimized) { animateRestore(win, this.buttonActor, this.dockUI.dockPosition); Main.activateWindow(win); } 
                    else if (isMaximized) { Main.activateWindow(win); win.unmaximize(); } 
                    else { Main.activateWindow(win); win.maximize(); }
                }));

                controlsBox.add_child(this._createWindowControl('window-close-symbolic', '255, 59, 48', () => {
                    if (!windows.includes(win)) return;
                    win.delete(global.get_current_time());
                    card.ease({ scale_x: 0, scale_y: 0, opacity: 0, duration: 200, onComplete: () => { if (card) card.destroy(); } });
        ...
        if (this.app.is_module && typeof this.app.open === 'function') {
            this.panel.add_child(this._createMenuItem(`Open ${this.app.get_name()}`, () => { this.app.open(); this.hide(); }));
            this._addSeparator();
        }
        ...
        if (typeof this.app.can_open_new_window === 'function' && this.app.can_open_new_window()) {
            this.panel.add_child(this._createMenuItem('New Window', () => {
                if (appInfo) appInfo.launch([], quietContext); else this.app.open_new_window(-1);
                this.hide();
            }));
            hasNewWindow = true;
        }
        ...
        if (actions.length > 0) {
            actions.forEach(action => {
                ...
                this.panel.add_child(this._createMenuItem(appInfo.get_action_name(action), () => { appInfo.launch_action(action, quietContext); this.hide(); }));
            });
        }
        ...
        if (this.isCtrlPressed && this.openPrefsCallback) {
            this._addSeparator();
            this.panel.add_child(this._createMenuItem('Dhruva Settings', () => { this.hide(); this.openPrefsCallback(); }));
        }
    }
    ...
    _applyThemeStyle(panel) {
        ...
        let themeCss = sWidth > 0 ? `border: ${sWidth}px solid ${hexToRgba(sColor, sOpacity)};` : 'border: none;';

        if (themeId === 'chameleon') {
            const { r, g, b } = this.dockUI._chameleonColor?.bg || { r: 30, g: 30, b: 45 };
            themeCss += ` background-color: rgba(${r}, ${g}, ${b}, 0.88);`;
        } else {
            const config = {
                opacity,
            ...
            themeCss += ` ${(DockThemes[themeId] || DockThemes['default']).css(config)}`;
        }

        panel.set_style(`${themeCss} padding: 12px; border-radius: 18px;`);
    }
    ...
    _createMenuItem(text, onClick, isDestructive = false) {
        ...
        btn.set_child(new St.Label({ text, style_class: isDestructive ? 'context-menu-action-label-destructive' : 'context-menu-action-label' }));
        btn.connect('clicked', onClick);
        return btn;
    }
    ...
    _createCheckboxItem(text, isChecked, onClick) {
        ...
        box.add_child(new St.Label({ text, style_class: 'context-menu-action-label', y_align: Clutter.ActorAlign.CENTER }));
        btn.set_child(box);
        btn.connect('clicked', onClick);
        return btn;
    }
