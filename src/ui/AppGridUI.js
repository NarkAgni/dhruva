...
export default class AppGridUI {
    constructor(dockUI) {
        ...
        let leftSpacer = new St.Widget({ x_expand: true });
        ...
        this.toggleViewBtn = new St.Button({
            child: new St.Icon({ icon_name: this.isGridView ? 'view-list-symbolic' : 'view-grid-symbolic', icon_size: 18 }),
        ...
        this.toggleViewBtn.connect('notify::hover', () => {
            this.toggleViewBtn.set_style(this.toggleViewBtn.hover ? 'padding: 6px; border-radius: 6px; background-color: rgba(255,255,255,0.25); transition-duration: 150ms;' : 'padding: 6px; border-radius: 6px; background-color: rgba(255,255,255,0.1); transition-duration: 150ms;');
        });

        this.toggleViewBtn.connect('clicked', () => {
            this.isGridView = !this.isGridView;
            this.toggleViewBtn.get_child().set_icon_name(this.isGridView ? 'view-list-symbolic' : 'view-grid-symbolic');
            this._filterApps(this.searchEntry.get_text());
            this._updatePosition();
        });

        let rightContainer = new St.BoxLayout({ x_expand: true, x_align: Clutter.ActorAlign.END });
        ...
        this.searchEntry.clutter_text.connect('key-press-event', (actor, event) => {
            ...
            let visibleRows = this.appRows;
            if (visibleRows.length === 0) return Clutter.EVENT_PROPAGATE;

            let cols = this.isGridView ? 5 : 1;
    ...
    _createCrispText(text, fontSize, isGrid) {
        let drawingArea = new St.DrawingArea({
            y_align: Clutter.ActorAlign.CENTER,
            x_align: isGrid ? Clutter.ActorAlign.FILL : Clutter.ActorAlign.START,
            x_expand: true
        });

        if (isGrid) {
            drawingArea.style = 'margin-top: 8px;';
        }

        let layout = drawingArea.create_pango_layout(text);
        let desc = Pango.FontDescription.from_string(`Sans Bold ${fontSize}px`);
        layout.set_font_description(desc);
        layout.set_ellipsize(Pango.EllipsizeMode.END);

        const gridFixedWidth = 88;

        if (isGrid) {
            layout.set_width(gridFixedWidth * Pango.SCALE);
        ...
        let updateDims = () => {
            ...
            let finalW = isGrid ? gridFixedWidth : Math.ceil(tw + 2);
            drawingArea.set_size(finalW, Math.ceil(th + 2));
        };
        updateDims();
        ...
        drawingArea.connect('repaint', (area) => {
            ...
            let [, th] = layout.get_pixel_size();

            let x = 0; 
            let y = Math.floor((h - th) / 2);
    ...
    _createListItem(app, name) {
        let btn = new St.Button({ style_class: 'app-list-click-area', reactive: true, x_expand: true, track_hover: true, style: 'border-radius: 8px;' });
        let content = new St.BoxLayout({ vertical: false, y_align: Clutter.ActorAlign.CENTER, x_expand: true, style: 'padding: 6px 4px;' });
        let icon = app.create_icon_texture(32);

        let nameLabel = this._createCrispText(name, 14, false);
    ...
    _createGridItem(app, name) {
        let btn = new St.Button({ style_class: 'app-grid-click-area', reactive: true, track_hover: true, style: 'border-radius: 12px; padding: 12px 4px;' });
        btn.set_size(96, 104);
        
        let content = new St.BoxLayout({ vertical: true, y_align: Clutter.ActorAlign.CENTER, x_align: Clutter.ActorAlign.FILL });
        let icon = app.create_icon_texture(48);

        let nameLabel = this._createCrispText(name, 13, true);
    ...
    _attachAppClickEvents(btn, app) {
        btn.connect('button-press-event', (actor, event) => {
            if (event.get_button() === 3) {
                ...
                let [, y] = btn.get_transformed_position();
                let dynamicDockPos = (y > (global.stage.height / 2)) ? 'BOTTOM' : 'TOP';
    ...
    _filterApps(searchText) {
        ...
        let visibleApps = this.allAppsData.filter(item => query === '' || item.lowerName.includes(query));
        let visibleCount = visibleApps.length;
        ...
        if (this.isGridView) {
            ...
            visibleApps.forEach((item, index) => {
                if (index % cols === 0) {
                    rowBox = new St.BoxLayout({ vertical: false, style: 'spacing: 4px; margin-bottom: 4px;', x_align: Clutter.ActorAlign.CENTER });
                    this.listContainer.add_child(rowBox);
                }
                let widget = this._createGridItem(item.app, item.name);
                rowBox.add_child(widget);
                this.appRows.push({ app: item.app, widget: widget, row: rowBox });
            });
        } else {
            visibleApps.forEach(item => {
                let widget = this._createListItem(item.app, item.name);
                this.listContainer.add_child(widget);
                this.appRows.push({ app: item.app, widget: widget, row: widget });
            });
        }
        ...
        if (this.isGridView) {
            let numRows = Math.ceil(visibleCount / 5);
    ...
    _updateSelection() {
        ...
        if (this.selectedIndex >= 0 && this.selectedIndex < this.appRows.length) {
            let target = this.appRows[this.selectedIndex];
            if (this.isGridView) {
    ...
    _scrollToItem(button) {
        ...
        this._scrollIdleId = GLib.idle_add(GLib.PRIORITY_LOW, () => {
            ...
            try {
                ...
                let targetActor = (this.isGridView && button.get_parent && button.get_parent())
                    ? button.get_parent()
                    : button;

                let allocation = targetActor.get_allocation_box();
    ...
    _applyChameleonStyle() {
        try {
            ...
            this.searchEntry.set_style(`
                ...
                box-shadow: none; 
            `);
        } catch (e) { console.error("Theme matching failed:", e); }
    }
    ...
    _updatePosition(overrideDockPos) {
        ...
        let panelW = this.isGridView ? 575 : 380;
