...
export function applyRealtimeFrame(dockActor, cx, cy, isVertical, settings, now = null) {
    try {
        ...
        if (!slotModel?.orderedSlots || slotModel.orderedSlots.length !== n) return;
        ...
        if (isHovering) {
            ...
            if (appName) {
                ...
                if (!dockActor._magTooltip) {
                    ...
                    if (!dockActor._magDestroyHandlerId) {
                        dockActor._magDestroyHandlerId = dockActor.connect('destroy', () => {
                            if (dockActor._magTooltip) {
                                try { dockActor._magTooltip.destroy(); } catch (_e) {}
                ...
                const appId = btn._delegate?.app ? (typeof btn._delegate.app.get_id === 'function' ? btn._delegate.app.get_id() : btn._delegate.app.get_name()) : (btn._delegate?.isFolder ? btn._delegate.folderData.id : appName);
                ...
                if (dockActor._tooltipReady && (!dockActor._magTooltip.visible || dockActor._magTooltip.opacity === 0)) {
                    ...
                    if (parent && dockActor._dockUI?.actor) {
                        try {
                            const sibling = dockActor._dockUI.actor;
                            if (sibling?.get_parent?.() === parent) {
...
export function setupMagnification(dockActor, settings, dockPositionGetter) {
    ...
    } catch (_e) { dockActor._magEnterId = null; }
    ...
    } catch (_e) { dockActor._magLeaveId = null; }
    ...
    dockActor._stageClickId = global.stage.connect('captured-event', (_stage, event) => {
        try {
            ...
            if (evType === Clutter.EventType.MOTION) {
                ...
                if (pos === 'BOTTOM' && onDock && dockActor._tooltipBridgeActive && dockActor._magTooltip?.visible) {
                    hideTooltip(dockActor);
                    return Clutter.EVENT_PROPAGATE;
                }
            ...
            const invokeButton = (targetBtn) => {
                if (!targetBtn || typeof targetBtn._activateCallback !== 'function') return false;
            ...
            const boxY = dockActor.boxActor ? dockActor.boxActor.y : 0;
            const slotModel = getFixedSlots(dockActor, isVertical, btns);
            if (!slotModel?.orderedSlots || slotModel.orderedSlots.length !== n) return Clutter.EVENT_PROPAGATE;
...
export function teardownMagnification(dockActor, skipReset = false) {
    ...
    if (isActorAlive(dockActor)) {
        try {
            if (dockActor._magMotionId) dockActor.disconnect(dockActor._magMotionId);
        } catch (_e) {}
        dockActor._magMotionId = null;

        try {
            if (dockActor._magLeaveId) dockActor.disconnect(dockActor._magLeaveId);
        } catch (_e) {}
        dockActor._magLeaveId = null;

        try {
            if (dockActor._magEnterId) dockActor.disconnect(dockActor._magEnterId);
        } catch (_e) {}
    ...
    if (isActorAlive(dockActor)) {
        try {
            if (dockActor._magDestroyHandlerId) dockActor.disconnect(dockActor._magDestroyHandlerId);
        } catch (_e) {}
    }
    dockActor._magDestroyHandlerId = null;
