...
export function createTooltipActor() {
    ...
    const tooltipBox = new St.BoxLayout({
...
export function populateTooltipContent(dockActor, btn, appName, settings) {
    ...
    else if (dockPos === 'RIGHT') padRight += ARROW_HEIGHT;
    ...
    if (windows.length > 0) {
        ...
        windows.forEach(win => {
            ...
            const thumbBtn = new St.Button({
                ...
                style: 'border-radius: 8px; background-color: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); transition-duration: 150ms;'
            });
            ...
            thumbContainer.add_child(thumbBtn);
            thumbContainer.add_child(labelBin);
            thumbContainer.add_child(controlsBin);
        ...
        if (windows.length > 2) {
            ...
            scroll.connectObject('scroll-event', (_actor, event) => {
                ...
                else if (dir === Clutter.ScrollDirection.UP) dy = -1;
                else if (dir === Clutter.ScrollDirection.DOWN) dy = 1;
                else if (dir === Clutter.ScrollDirection.LEFT) dx = -1;
                else if (dir === Clutter.ScrollDirection.RIGHT) dx = 1;
                ...
                if (dx !== 0) {
                    ...
                    if (adj) {
                        const step = dir === Clutter.ScrollDirection.SMOOTH ? dx * 40 : dx * 50;
                        const newVal = adj.get_value() + step;
                        const min = adj.get_lower();
    ...
    const trackPos = () => {
        ...
        const [, tw] = dockActor._magTooltipBox.get_preferred_width(-1);
        const [, th] = dockActor._magTooltipBox.get_preferred_height(-1);
        ...
        if (dockPosStr === 'BOTTOM' || dockPosStr === 'TOP') {
